...
class FileData extends BuildResponse {
  ...
  FileData({@required this.file, @required request, this.mime, this.headers, this.maxAge}) : super(request: request);

  factory FileData.path(String path,
          {@required Request request, String mime, Map<String, String> headers, int maxAge}) =>
  ...
  Future<Response> builder() async {
    ...
    if (file != null && await file.exists()) {
      if (request.headers.containsKey('If-Modified-Since') &&
          request.headers['If-Modified-Since'] == formatHttpDate(await file.lastModified())) {
...
//            'Cache-Control': 'max-age=${maxAge ?? 86400}, public',
        ...
        return Response.internalServerError(
            body: 'Server error: $e', headers: {'Content-Type': MimeTypes.html, ...?headers});
      }
    }
    return Response.notFound('File not found: ${request.url}', headers: {'Content-Type': MimeTypes.html, ...?headers});
  }
}
