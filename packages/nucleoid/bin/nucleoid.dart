...
void _help(String? action) {
  ...
  print(
    '1. help nucleoid commands:                    \$ pub run nucleoid help',
  );
  print(
    '2. build a standalone executable:             \$ pub run nucleoid build',
  );
  print(
    '           with target dart file:             \$ pub run nucleoid build -t bin/server.dart',
  );
  print(
    '3. clean builds and temp files:               \$ pub run nucleoid clean',
  );
  print('');
}
...
Future<void> _build(ArgResults args) async {
  ...
  try {
    ...
    final binaryFileName = [
      'app',
      if (configYaml != null && configYaml.containsKey('name'))
        configYaml['name'],
      if (Platform.isWindows) 'exe',
    ].join('.');
    final currentDir = Directory.current;
    final tempDir = Directory(
      '${currentDir.path}${Platform.pathSeparator}$_buildTempDir',
    );
    final buildDir = Directory(
      '${currentDir.path}${Platform.pathSeparator}$_buildDir',
    );
    final tempFileOutput =
        '${tempDir.path}${Platform.pathSeparator}$binaryFileName';
    final buildFileOutput =
        '${buildDir.path}${Platform.pathSeparator}$binaryFileName';

    if (!tempDir.existsSync()) tempDir.createSync(recursive: true);

    if (configYaml != null && configYaml.containsKey('nucleoid')) {
      if (configYaml['nucleoid'] is YamlMap &&
          configYaml['nucleoid'].containsKey('assets')) {
        _copyAssets(tempDir, configYaml['nucleoid']['assets']);
      }

      if (configYaml['nucleoid'] is YamlMap &&
          configYaml['nucleoid'].containsKey('scripts')) {
        await _buildScripts(tempDir, configYaml['nucleoid']['scripts']);
      }
    }

    /// Run: dart2native
    var result2native = Process.runSync('dart compile exe', [
      args['target'] ?? 'bin/main.dart',
      '-o',
      tempFileOutput,
    ], runInShell: true);

    if (result2native.stdout.toString().isNotEmpty)
      print('dart2native result: ${result2native.stdout}');
...
void _killWindowsProcess(String fileProcess) {
  var resultKill = Process.runSync('taskkill', [
    '/FI',
    'WINDOWTITLE eq $fileProcess',
  ], runInShell: true);

  if (resultKill.stdout.toString().isNotEmpty)
    print('taskkill result: ${resultKill.stdout}');
  if (resultKill.stderr.toString().isNotEmpty)
    print('Error taskkill: ${resultKill.stderr.toString()}');
}

void _killLinuxProcess(String fileProcess) {
  var resultKill = Process.runSync('bash', [
    '-c',
    'kill -9 \$(ps faux | grep -w \'$fileProcess\' | grep -vw grep | awk \'{ print \$2 }\')',
  ], runInShell: true);
...
void _copyAssets(Directory dir, YamlList list) {
  list.forEach((asset) {
    ...
    if (dirElement.existsSync()) {
      copyPathSync(
        dirElement.path,
        '${dir.path}${Platform.pathSeparator}$asset',
      );
    } else if (fileElement.existsSync()) {
      var dirElement = Directory(
        '${dir.path}${Platform.pathSeparator}${fileElement.parent.path}',
      );
