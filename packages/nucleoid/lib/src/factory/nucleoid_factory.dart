...
class FactoryNucleoid {
    ...
    map.forEach((key, value) {
      value.forEach((factory) {
        if (factoryMap.containsKey(factory))
          throw Exception('FactoryNucleoid: factory "$factory" init before.');
  ...
  Future<void> test(
    List<FunctionTestFactory> tests, {
    required bool autoCorrect,
  }) => Future.forEach<FunctionTestFactory>(
    tests,
    (test) => test(autoCorrect: autoCorrect),
  );
